...
import { ta, Series, math, type IndicatorResult, type InputConfig, type PlotConfig, type Bar } from 'oakscriptjs';
...
function calculateQQE(
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i === 0) {
      ...
      // Cross detection
      const crossShort = (prevSr <= shortBand[i - 1] && sr > shortBand[i - 1]) ||
                          (prevSr >= shortBand[i - 1] && sr < shortBand[i - 1]);
