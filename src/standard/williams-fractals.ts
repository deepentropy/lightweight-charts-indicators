...
// No line plots — rendered via extended markers (triangleUp/triangleDown)
...
export function calculate(bars: Bar[], inputs: Partial<WilliamsFractalsInputs> = {}): IndicatorResult & { markers: MarkerData[] } {
  ...
  // Convert fractal points to extended markers
  const markers: MarkerData[] = [];

  for (let i = 0; i < bars.length; i++) {
    if (!isNaN(upFractalValues[i])) {
      markers.push({
